...
export default function CourseCatalog() {
  ...
  function getTypeLabel(course: typeof courses[number]) {
    if (course.category === 'POS_GRADUACAO') return 'Pós-graduação Digital';
    return course.type;
  }
  ...
  return <>
    ...
    {filtered.length ? <div className="course-grid">{filtered.map(course => {
      ...
      const isPost = course.category === 'POS_GRADUACAO';
      const isGrad = course.category === 'GRADUACAO';
      return <article className="card course-card" key={course.id}>
        <div className="course-image"/>
        <span className="tag">{getTypeLabel(course)}</span>
        <h3>{course.name}</h3>
        <div className="course-meta"><Building2 size={14}/> {institution} <span>•</span> {course.area}</div>
        ...
        <Link className="btn btn-dark" href={`/curso/${course.slug}`}>
          {isPost ? 'Ver detalhes da pós' : isGrad ? 'Ver detalhes da graduação' : course.institution === 'IBESC' ? 'Saiba mais' : 'Ver formação'}
        </Link>
      </article>;
