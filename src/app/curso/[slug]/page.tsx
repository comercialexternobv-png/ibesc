...
export default async function CoursePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main>
    ...
    <section className="section"><div className="container contact-grid"><div>
      ...
      <div className="diff-grid" style={{ marginTop: 24 }}><div className={`card diff-card ${styles.courseDetailCard}`}><Clock3 /><h3>{isPost ? 'Modalidade digital' : 'Formato da formação'}</h3><p>{isPost ? 'A formação é disponibilizada digitalmente pela instituição responsável.' : c.modality || 'Consulte o consultor IBESC para confirmar o formato e a dinâmica da turma.'}</p></div><div className={`card diff-card ${styles.courseDetailCard}`}><Users /><h3>Para quem é</h3><p>{audience}</p></div><div className={`card diff-card ${styles.courseDetailCard}`}><BookOpen /><h3>O que você aprende</h3><p>{learning[0]}</p></div></div>
