  ...
  return <main>
    <section className="hero"><div className="container hero-grid"><div>
      ...
      <div className="hero-actions">
        ...
        {isPost && <a className="btn btn-outline" href={`https://wa.me/${wa}?text=${msg}`}>Falar com um consultor <MessageCircle size={17} /></a>}
    ...
    </div><div className="hero-card" style={{overflow:'hidden',padding:0}}>{c.image ? <img src={c.image} alt={`Imagem de ${c.name}`} style={{width:'100%',height:300,objectFit:'cover',display:'block'}} /> : <div className="fake-photo" />}<div style={{padding:'18px 20px'}}><strong>{isPost ? `Pós-graduação Digital ${institutionLabel}` : own ? 'Formação própria IBESC' : `Formação em parceria com ${institution}`}</strong></div></div></div></section>

    {c.images && c.images.length > 0 && <section className="section" style={{paddingBottom:0}}><div className="container"><div className="section-head"><span className="eyebrow">Conheça a estrutura</span><h2>Veja o Técnico em Enfermagem de perto</h2><p>Conheça os ambientes e momentos de aprendizagem apresentados pelo IBESC.</p></div><div style={{display:'grid',gridTemplateColumns:'repeat(2,minmax(0,1fr))',gap:18}}>{c.images.map((image, index) => <div key={image} className="card" style={{padding:0,overflow:'hidden',borderRadius:16}}><img src={image} alt={`${c.name} — imagem ${index + 1}`} style={{display:'block',width:'100%',height:280,objectFit:'cover'}} /></div>)}</div></div></section>}
    ...
    <section className="section"><div className="container contact-grid"><div>
      ...
      <div className="diff-grid" style={{ marginTop: 24 }}><div className="card diff-card"><Clock3 /><h3>{isPost ? 'Modalidade digital' : 'Formato da formação'}</h3><p>{isPost ? 'A formação é disponibilizada digitalmente pela instituição responsável.' : c.modality || 'Consulte o consultor IBESC para confirmar o formato e a dinâmica da turma.'}</p></div><div className="card diff-card"><Users /><h3>Para quem é</h3><p>{audience}</p></div><div className="card diff-card"><BookOpen /><h3>O que você aprende</h3><p>{learning[0]}</p></div></div>
